...
[![PyPI](https://img.shields.io/pypi/v/snapshot-testing?label=snapshot-testing)](https://pypi.org/project/snapshot-testing/)
...
Snapshot tests capture the output of your program and compare it against a stored "snapshot".

This means:

- :negative_squared_cross_mark: No need to write elaborate assertions
- :star2: Program output is captured in a human and agent-friendly format
- :100: Tests cover logging output, warnings, and exceptions that is otherwise easily ignored
...
When the test `test_func` is run, it will create a snapshot file storing the output of the test:
...
Once you are happy with the output, commit the snapshot to version control.

### Snapshots also reflect logs, warnings & exceptions
...
def test_outs() -> None:
    ...
    warnings.warn("This is a warning message.", stacklevel=2)
...
This package aims to provide a simple snapshot testing solution that requires minimal setup and reduces the time to
develop tests.
...
Since the snapshots generated by this package capture all output, including logs and exceptions tests should not print
...
The `@snapshot` decorator can be applied to tests that use `pytest` fixtures.
...
@snapshot
def test_with_fixture(sample_data: dict[str, int]):
    print(sample_data)
```
...
The `@snapshot` decorator also works with parametrized tests.
...
def test_parametrized(input_value: int):
    print(f"Input value is: {input_value}")
```

Will produce snapshots like:

```text
# snapshots/test_parametrized[1].snap
Input value is: 1
```

```text
# snapshots/test_parametrized[2].snap
Input value is: 2
```
